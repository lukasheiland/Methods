...
opt <- optim(1.0, func) # default Nelder-Mead; Newton method, e.g., method = 'BFGS' 
opt$par # very close to 0


# Optimizytion of linear regressin with brute force -----------------------
data <- airquality[complete.cases(airquality$Ozone) & complete.cases(airquality$Solar.R),]
X <- scale(data[,-1])
Y <- data$Ozone

linear_regression <- function(beta) {
  pred <- beta[1]*X[,1] + # Solar.R
          beta[2]*X[,2] + # Wind
          beta[3]*X[,3] + # Temp
          beta[4]*X[,4] + # Month
          beta[5]*X[,5] +
          beta[6]         # or X %*% w[1:5] + w[6]
  
  ## loss: MSE, we want to find the optimal weights to minimize the sum of squared residuals
  loss <- mean((pred - Y)^2)
  
  ## alternatively likelihood: sum(dnorm), with sd as parameter
  # negLL <- - sum(dnorm(data$Ozone - pred, mean = 0, sd = par['sd'], log = T))
...
## Random bruteforce optimization:
